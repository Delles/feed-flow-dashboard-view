    ...
    return (
        <div
            className={`transition-all duration-500 ${
                !isFeedEnabled || shouldDim ? "opacity-30 grayscale" : "opacity-100"
            }`}
        >
            <button
                onClick={() => {
                ...
                className={`w-full group relative px-4 py-2 rounded-xl transition-all duration-500 ${
                    isSelected
            ...
            >
                <div className="flex items-center w-full gap-3">
                    ...
                    <div className="flex-1 min-w-0 flex items-center justify-between gap-2">
                        <h4
                            className={`font-sans font-bold text-[13px] tracking-tight truncate transition-colors duration-300 ${
                                isSelected ? "text-primary" : "text-foreground/80 dark:text-foreground/70 group-hover:text-foreground"
                            }`}
                        >
                            {feed.title}
                        </h4>
                                ...
                                <TooltipTrigger asChild>
                                    <button
                                        ...
                                        className={`transition-all duration-300 p-1 rounded-md ${
                                            isSelected ? "text-primary hover:bg-primary/10" : "text-muted-foreground/60 dark:text-foreground/40 hover:text-primary hover:bg-primary/5"
                                        }`}
                                    >
                                        {isItemEnabled ? (
